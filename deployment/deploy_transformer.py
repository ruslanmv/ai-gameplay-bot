...
model = GameplayTransformer(INPUT_SIZE, NUM_HEADS, HIDDEN_SIZE, NUM_LAYERS, OUTPUT_SIZE)
...
def predict_action(state):
    """
    Predict the action based on the input state using the pre-trained Transformer model.
    Args:
...
def health():
    ...
    return jsonify({"status": "healthy", "service": "transformer"}), 200
